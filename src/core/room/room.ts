...
export abstract class Room<T extends WorkPlace = WorkPlace> {
  ...
  //Server only
  public abstract trigger(content: ServerEventFinder<GameEventIdentifiers>, stage?: AllStage): void;
  //Server only
  ...
  public changeSeals(playerId: PlayerId, seals: Seal[], method: SealMethod) {
    switch (method) {
      case SealMethod.Clear:
        this.getPlayerById(playerId).clearSeals();
        break;
      case SealMethod.Add:
        this.getPlayerById(playerId).addSeals(seals);
        break;
      case SealMethod.Remove:
        this.getPlayerById(playerId).removeSeals(seals);
        break;
      default:
        throw new Error('unhandleable seal method: ' + method);
    }
  }
  ...
  public getRoundDropCards(): Card[] {
    const records = this.Analytics.getRecordEvents<GameEventIdentifiers.MoveCardEvent>(
      event =>
