...
import {
  MoveCardEventInfos,
  MovingCardProps,
} from "src/core/event/event.server";
...
export class StandardGameProcessor extends GameProcessor {
  ...
  public assignRoles(players: Player[]) {
    ...
    const postions = Array.from(Array(players.length - 1), (_, k) => k + 1);
    Algorithm.shuffle(postions);
    for (let i = 0; i < players.length; i++) {
      if ((players[i].Role = PlayerRole.Lord)) {
        players[i].Position = 0;
      } else {
        players[i].Position = postions.pop()!;
      }
    }
  }
    ...
    if (lordDied) {
      ...
      return players.filter(
        (player) =>
          ...
          player.Role === PlayerRole.Loyalist,
      );
    }
  }
